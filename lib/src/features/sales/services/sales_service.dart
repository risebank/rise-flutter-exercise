...
class SalesService {
  ...
  /// Fetch parties for recipient selection (create invoice).
  /// Endpoint: GET /companies/{company_id}/parties?scope=associated
  Future<ApiResponse<List<PartyListItemModel>>> fetchParties(
    BuildContext context,
    String companyId, {
    String scope = 'associated',
    String? query,
    int offset = 0,
    int limit = 50,
  }) async {
    final endpoint = Endpoints.parties(companyId);
    ...
    return ApiResponse.fromApiClientResponse(
      ...
      parser: (json) => (json as List)
          .map((e) => PartyListItemModel.fromJson(e as Map<String, dynamic>))
          .toList(),
      errorMessage: ErrorMessages.fetchError(context, 'parties'),
    );
  }
